...
int main(int argc, const char* argv[])
{
    ...
    //touch.execute();
    std::ofstream m;
    m.open(directory + "/main.cpp");
    m << "#include <iostream>\n\n";
    m << "int main()\n" << "{\n" << "    ";
    m << "std::cout << \"Hello, World!\\n\";\n";
    m << "    return 0;\n}";
    m.close();

    std::string makefile = "SOURCES := $(wildcard *.cpp)\n";
    makefile += "OBJECTS := $(patsubst %.cpp,%.o,$(SOURCES))\n";
    makefile += "DEPENDS := $(patsubst %.cpp,%.d,$(SOURCES))\n\n";
